...
#define S 10000
#define DCombEstadisticas 250000
#define DDebug 0
...
int main(int argc, char *argv[])
{
	...
	if (argc>2)
		EstadisticasParciales = atoi(argv[2]);
	else 
		EstadisticasParciales = DCombEstadisticas;
...
bool CalcularCombinacionOptima(int PrimeraCombinacion, int UltimaCombinacion, PtrListaArboles Optimo)
{
	...
	for (Combinacion=PrimeraCombinacion; Combinacion<UltimaCombinacion; Combinacion++, EstCombinaciones++)
	{
...
void InicializarEstadisticas()
{
	...
	EstCombInvalidas=0; 
	EstCosteTotal=0;
	EstMejorCosteCombinacion=DMaximoCoste;
	EstPeorCosteCombinacion=0;
	EstMejorArboles=ArbolesEntrada.NumArboles;
	EstPeorArboles=0;
	EstMejorArbolesCombinacion=0;
	EstPeorArbolesCombinacion=0;
	EstMejorCoste=DMaximoCoste;
	EstPeorCoste=0;
}
...
{
	printf("\n++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++\n");
	printf("++ Eval Comb: %d \tValidas: %d \tInvalidas: %d\tCoste Validas: %.3f\n", EstCombinaciones, EstCombValidas, EstCombInvalidas, (float)EstCosteTotal/(float)EstCombValidas);
	printf("++ Mejor Comb (coste): %d Coste: %.3f  \tPeor Comb (coste): %d Coste: %.3f\n",EstMejorCosteCombinacion, EstMejorCoste, EstPeorCosteCombinacion, EstPeorCoste);
	printf("++ Mejor Comb (árboles): %d Arboles: %d  \tPeor Comb (árboles): %d Arboles %d\n",EstMejorArbolesCombinacion, EstMejorArboles, EstPeorArbolesCombinacion, EstPeorArboles);
	printf("++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++\n");
}
...
if (DDebug) printf("\tCoste:%d",DMaximoCoste);
	...
	// Estadisticas Combinaciones.
	EstCombValidas++;
	EstCosteTotal+=CosteCombinacion;
	if (CosteCombinacion<EstMejorCoste)
	{
		EstMejorCoste = CosteCombinacion;
		EstMejorCosteCombinacion =  Combinacion;
	}
	else if (CosteCombinacion>EstPeorCoste)
	{
		EstPeorCoste = CosteCombinacion;
		EstPeorCosteCombinacion =  Combinacion;
	}
	if (NumArboles<EstMejorArboles)
	{
		EstMejorArboles = NumArboles;
		EstMejorArbolesCombinacion =  Combinacion;
	}
	else if (NumArboles>EstPeorArboles)
	{
		EstPeorArboles = NumArboles;
		EstPeorArbolesCombinacion =  Combinacion;
	}
