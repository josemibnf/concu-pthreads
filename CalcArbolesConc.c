...
bool CalcularCercaOptima(PtrListaArboles Optimo)
{
	...
	int bloque=MaxCombinaciones/numHilos + 1;  //Determina el tamaño de los conjuntos de combinaciones que se asignan a cada hilo.
	...
	PtrListaArboles Parcial[numHilos];  //Optimo parcial.

	while (i<MaxCombinaciones){
		
		int bloqueS; //Determina cual debe de ser la Ultima Combinacion del siguiente bloque.
		if(i+bloque > MaxCombinaciones){
			bloqueS = MaxCombinaciones;
		}else{								//No cumplira la condicion anterior siempre y cuando le sobren conbinaciones al ultimo bloque.
			bloqueS = i+bloque;					//El algoritmo de reparticion de bloques se explica mas detalladamente en el informe.
		}
		
		Parcial[j] = malloc(sizeof(TListaArboles)); //Reserva memoria dinamicamente para el optimo parcial del hilo j.

		Parcial[j]->Coste=DMaximoCoste;
		Parcial[j]->NumArboles=0;
		struct args *Argumentos = (struct args *)malloc(sizeof(struct args));   //Reserva memoria para los atributos del metodo que ejecutara el hilo.
			...
			Argumentos->arg2=bloqueS;	//Ultima combinacion del bloque.
			Argumentos->arg3=Parcial[j];  //Optimo parcial.
		if(pthread_create(&hilos[j], NULL, CalcularCombinacionOptima, (void *)Argumentos)){
			printf("Error al crear hilo\n");
			exit(1);
		}
		j++; i=bloqueS; //Se aumenta j y se iguala i 
	}

	j=0; i=0;  	//Recorremos los bloques de la manera anterior.
	while(i<MaxCombinaciones){
		
		if(pthread_join(hilos[j], NULL)){		//Espera a que termine la ejecucion del hilo j.
			printf("Error al hacer join\n");
			exit(1);
		}
		
		if ( Parcial[j]->Coste < Optimo->Coste )	//En el caso en que el coste del optimo parcial supere al global, se sustituyen toodos sus atributos.
		{
			Optimo->Coste = Parcial[j]->Coste;
			Optimo->NumArboles = Parcial[j]->NumArboles;
			Optimo->CosteArbolesCortados = Parcial[j]->CosteArbolesCortados;
			Optimo->CosteArbolesRestantes = Parcial[j]->CosteArbolesRestantes;
			Optimo->LongitudCerca = Parcial[j]->LongitudCerca;
			Optimo->MaderaSobrante = Parcial[j]->MaderaSobrante;
			int z; for(z=0; z<Optimo->NumArboles; z++){			//Se pasa todo el vector de arboles.
				Optimo->Arboles[z] = Parcial[j]->Arboles[z];
			}
		}
		free(Parcial[j]);  //Se libera memoria.
		j++; i=i+bloque;
	}
	return true;
}
 ...
 void* CalcularCombinacionOptima(void *input)
{
	...
	PtrListaArboles Optimo = ((struct args*)input)->arg3;
	...
	for (Combinacion=PrimeraCombinacion; Combinacion<=UltimaCombinacion; Combinacion++) //Recorremos todas las combinaciones, contando tambien la ultima.
	{
	//printf("\tC%d -> \t",Combinacion);
		Coste = EvaluarCombinacionListaArboles(Combinacion);  //Se almacena el coste de la combinacion.
 	//printf("             %d\n",Coste);
